...
const unitName = computed(() => {
  if (props.theme === 'unit00') return 'UNIT-00'
  if (props.theme === 'unit02') return 'UNIT-02'
  return 'UNIT-01'
})
      ...
      <div class="central-hud" :class="{ 'hud-visible': isVisible }">
        ...
        <div class="content-core">
          <div class="top-meta">
            ...
            <!-- 修复：动态显示当前机体编号 -->
            <span class="meta-text">TARGET_LOCKED: {{ unitName }}_PILOT</span>
          </div>
...
.hero-view { height: 100vh; width: 100%; display: flex; align-items: center; justify-content: center; font-family: 'Share Tech Mono', monospace; overflow: hidden; position: relative; }
* { color: var(--text-main); }
.hud-alignment-wrapper { width: 100%; display: flex; flex-direction: column; align-items: center; margin-top: 18vh; }
.bg-watermark { position: absolute; font-family: 'Orbitron', sans-serif; font-size: 20vw; font-weight: 900; color: var(--accent); opacity: 0.03; user-select: none; pointer-events: none; z-index: 1; left: 5%; bottom: 10%; }
.central-hud { position: relative; z-index: 10; width: 90%; max-width: 750px; padding: 50px 60px; opacity: 0; transform: scale(0.95); transition: all 1s cubic-bezier(0.23, 1, 0.32, 1); box-sizing: border-box; }
...
.scan-line { position: absolute; left: 0; width: 100%; height: 2px; background: var(--accent); opacity: 0.2; box-shadow: 0 0 15px var(--accent); animation: scan-loop 4s linear infinite; }
...
.top-meta { display: flex; align-items: center; gap: 10px; margin-bottom: 20px; background: var(--eva-red); color: #fff !important; padding: 4px 15px; font-weight: 950; font-size: 0.8rem; letter-spacing: 2px; }
.top-meta * { color: #fff; }
...
.pilot-name { font-family: 'Orbitron', sans-serif; font-size: clamp(3rem, 10vw, 6.5rem); font-weight: 900; margin: 0; line-height: 0.9; letter-spacing: -4px; text-shadow: 0 0 30px rgba(var(--accent), 0.2); }
.bio-typing { font-size: 1.2rem; margin-top: 20px; font-weight: 600; opacity: 0.8; width: 100%; border-bottom: 2px solid var(--accent); padding-bottom: 12px; text-align: center; }
...
.cf-stats-row { display: flex; justify-content: center; align-items: center; gap: clamp(20px, 5vw, 40px); }
...
.mission-trigger { position: absolute; bottom: 40px; cursor: pointer; display: flex; flex-direction: column; align-items: center; transition: 0.3s; }
...
.arrow-down { width: 24px; height: 12px; background: var(--accent); clip-path: polygon(0 0, 100% 0, 50% 100%); margin-bottom: 10px; animation: bounce-arrow 2s infinite; }
@keyframes bounce-arrow { 0%, 100% { transform: translateY(0); } 50% { transform: translateY(5px); } }
@media (max-width: 768px) { .central-hud { padding: 30px; width: 95%; } .hud-alignment-wrapper { margin-top: 10vh; } .pilot-name { font-size: 2.8rem; letter-spacing: -2px; } .bio-typing { font-size: 0.9rem; } .cf-stats-row { gap: 15px; } .stat-group .value { font-size: 1.1rem; } .chart-container { height: 70px; } }
</style>
